...
import { CreateSubCategories } from "./dto/create-subCategories";
...
> {
  ...
  async createWithBanners(data: CreateCategoriesDto){
  ...
  async changeStatus(id: string): Promise<boolean> {
   ...
   async createSub(CreateSubCategories){
   return await this.categoriesRepository.save(
      await this.categoriesRepository.create(CreateSubCategories
      )
    )
